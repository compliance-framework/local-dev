...
do
	set +e
	if ! plan_id="$(curl -s localhost:8080/api/plan --header 'Content-Type: application/json' -d '
{
  "title":
  "Demo Azure VM Tag Assessment Plan"
}
' | jq -r .id)"
	then
...
task_id="$(curl -s localhost:8080/api/plan/"${plan_id}"/tasks --header 'Content-Type: application/json' -d '
{
...
activity_id="$(curl -s localhost:8080/api/plan/"${plan_id}"/tasks/"${task_id}"/activities --header 'Content-Type: application/json' -d '
{

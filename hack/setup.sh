...
activity_id="$(curl -s localhost:8080/api/plan/"${plan_id}"/tasks/"${task_id}"/activities --header 'Content-Type: application/json' -d '{"title":"CheckVMsforport80", "description":"Thisactivitychecksfortheport", "provider":{"name":"azurecli", "package":"azurecli", "params":{"parameter1":"this-is-the-parameter-value"}, "configuration":{"subscriptionId":"'${AZURE_SUBSCRIPTION_ID}'"}, "version":"1.0.0"}, "subjects":{"title":"VMsunderFinancesubscription", "description":"Notallofthemachines", "labels":{"subscription":"finance", "env":"prod"}}}' | jq -r .id)"

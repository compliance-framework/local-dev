...
plan_id="$(curl -v localhost:8080/api/plan --header 'Content-Type: application/json' -d '{"title": "Sample Assessment Plan"}' | jq -r .id)"
echo "Plan ID: ${plan_id}"

echo "Creating Task: Check All the VMs"
task_id="$(curl -v localhost:8080/api/plan/"${plan_id}"/tasks --header 'Content-Type: application/json' -d '{"description": "Check All the VMs", "title": "VM Check", "type": "action", "schedule": "0 * * * * *"}' | jq -r .id)"

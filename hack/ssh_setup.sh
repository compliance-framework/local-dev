...
activity_id="$(curl -s localhost:8080/api/plan/"${plan_id}"/tasks/"${task_id}"/activities --header 'Content-Type: application/json' -d '{"title":"Check server is OK", "description":"This activity checks the server is OK", "provider":{"name":"ssh-cf-plugin", "package":"ssh-cf-plugin", "image":"ghcr.io/compliance-framework/ssh-cf-plugin", "params":{}, "configuration":{"username":"'${CF_SSH_USERNAME}'","password":"'${CF_SSH_PASSWORD}'","host":"'${CF_SSH_HOST}'","command":"'"$CF_SSH_COMMAND"'","port":"'${CF_SSH_PORT:-2227}'"}, "tag":"latest"}, "subjects":{"title":"Server", "description":"Server: '${CF_SSH_HOST}'", "labels":{}}}' | jq -r '.id')"

...
plan_id="$(curl -s localhost:8080/api/plan --header 'Content-Type: application/json' -d '{"title": "Sample SSH Assessment Plan"}' | jq -r .id)"
...
echo $response
activity_id="$(echo -n $response | jq -r .id)"
